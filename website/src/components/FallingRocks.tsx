...
// Deterministic PRNG (mulberry32) so the exact same seed always produces the
// exact same sequence of "random" values. The server picks one seed per
// request and passes it down as a prop; since both the server's render and
// the client's initial hydration pass use this same seed (instead of raw
// Math.random()), their output matches exactly - so this background can
// render for real during SSR with no hydration mismatch, while still coming
// out different on every fresh page load (a new seed each time).
...
// Wider than the lane itself (in vw, not px - see note on ROUTE_LAYOUT
// below for why everything here is viewport-relative rather than measured),
// so neighboring lanes' holds slightly overlap and read as one connected
// wall rather than isolated columns.
const LATERAL_OFFSET_VW = 6.94;
const JITTER_RANGE_VW = 2.64;

// Each route's horizontal position, per breakpoint, expressed as static
// Tailwind classes instead of a JS-measured pixel width. Routes reveal
// outward from the center as the viewport widens (exactly one visible on
// mobile), and at every tier the currently-visible set is spread evenly
// across the full width - e.g. route 2 sits at 50% alone on mobile, but at
// 25% once route 3 joins it, etc. Because this is a fixed mapping from
// breakpoint to position (not random), it can be precomputed once as
// literal class names, which lets the CSS resolve it immediately with no
// DOM measurement, no ResizeObserver, and no guessing an initial size.
const ROUTE_LAYOUT: string[] = [
  "hidden xl:block xl:left-[10%] 2xl:left-[8.3333%]",
  "hidden md:block md:left-[16.6667%] lg:left-[12.5%] xl:left-[30%] 2xl:left-[25%]",
  "block left-[50%] sm:left-[25%] md:left-[50%] lg:left-[37.5%] xl:left-[50%] 2xl:left-[41.6667%]",
  "hidden sm:block sm:left-[75%] md:left-[83.3333%] lg:left-[62.5%] xl:left-[70%] 2xl:left-[58.3333%]",
  "hidden lg:block lg:left-[87.5%] xl:left-[90%] 2xl:left-[75%]",
  "hidden 2xl:block 2xl:left-[91.6667%]",
];
...
function assignSegmentColours(count: number, firstColour: string, rng: Rng): string[] {
  const colours: string[] = [];
  let previousColour: string | null = null;

  while (colours.length < count) {
    const segmentLength = Math.min(count - colours.length, 24 + Math.floor(rng() * 9));
    let colour: string;
    if (previousColour === null) {
      colour = firstColour;
    } else {
      do {
        colour = HOLD_COLOURS[Math.floor(rng() * HOLD_COLOURS.length)];
...
function generateRouteBase(rng: Rng): RouteBase[] {
  const firstColours = shuffle(HOLD_COLOURS, rng).slice(0, ROUTE_COUNT);

  return Array.from({ length: ROUTE_COUNT }, (_, routeIndex) => {
    const pattern = ALL_PATTERNS[Math.floor(rng() * ALL_PATTERNS.length)];

    // Same average hold size as before (~108), but a tighter spread so the
    // maximum is 75% of what it used to be (~157.5 vs. ~210).
    const baseSize = 68.5 + rng() * 79;
    const rowColours = assignSegmentColours(HOLDS_PER_ROUTE, firstColours[routeIndex], rng);
...
function generateTextureSlots(rng: Rng): TextureSlot[] {
  return Array.from({ length: TEXTURE_SLOT_COUNT }, (_, i) => ({
    key: i,
    col: i % TEXTURE_COLS,
    sizeRatio: 0.55 + rng() * 0.3,
    jitterX: (rng() - 0.5) * 2,
    rotation: rng() * 360,
    opacity: 0.14 + rng() * 0.12,
    ...
    delay: -(i * TEXTURE_INTERVAL_S + (rng() - 0.5) * TEXTURE_INTERVAL_S * 0.4),
  }));
}
...
function generateWallFacets(rng: Rng): WallFacet[] {
  ...
  for (let row = 0; row <= WALL_FACET_ROWS; row++) {
    ...
    for (let col = 0; col <= WALL_FACET_COLS; col++) {
      ...
      const jitterY = isEdge ? 0 : (rng() - 0.5) * cellH * 0.7;
  ...
  for (let row = 0; row < WALL_FACET_ROWS; row++) {
    for (let col = 0; col < WALL_FACET_COLS; col++) {
      ...
      triangles.forEach((triangle, i) => {
        const lightness = WALL_BASE_LIGHTNESS + (rng() - 0.5) * WALL_LIGHTNESS_SPREAD;
        facets.push({
          key: `${row}-${col}-${i}`,
...
type GeneratedScene = {
  ...
  wallFacets: WallFacet[];
};

// Single self-contained entry point for all seeded randomness. This must be
// the ONLY thing wrapped in useMemo (not one memo per piece sharing one rng)
// - React's dev-mode Strict Mode double-invokes memo factories on the client
// to catch impurities, and since the rng returned by mulberry32 is a
// stateful closure, double-invoking some but not all of several memos that
// share one rng instance would advance its state inconsistently between the
// server's single pass and the client's, desyncing everything after that
// point. Bundling it all into one factory means each invocation (however
// many times React calls it) creates and fully consumes its own fresh rng
// from the same seed, so every call produces an identical result.
function generateScene(seed: number): GeneratedScene {
  const rng = mulberry32(seed);
  return {
    routes: generateRouteBase(rng),
    textureSlots: generateTextureSlots(rng),
    wallFacets: generateWallFacets(rng),
  };
}
...
export default function FallingRocks({ seed }: { seed: number }) {
  const [paused, setPaused] = useState(false);
  const animationPlayState = paused ? "paused" : "running";

  // Deterministic given the seed - identical on the server and the client,
  // so server-rendered HTML and the client's initial hydration pass produce
  // byte-for-byte the same "random" layout. See generateScene's comment for
  // why this must be a single memo rather than one per piece.
  const { routes, textureSlots, wallFacets } = useMemo(() => generateScene(seed), [seed]);

  // Percent-of-viewport-width cell, used for both position and size - no
  // container measurement needed since it's relative to the viewport, and
  // this hero section is always full-bleed width.
  const textureCellPercent = 100 / TEXTURE_COLS;

  const textureNodes = textureSlots.map((slot) => {
    const triSizeVw = textureCellPercent * slot.sizeRatio;
    const maxJitterVw = Math.max(0, ((textureCellPercent - triSizeVw) / 2) * 0.7);
    const centerPercent = (slot.col + 0.5) * textureCellPercent + slot.jitterX * maxJitterVw;
  ...
  const ropeNodes = routes.map((route, routeIndex) => (
    <div
      key={route.key}
      className={`animate-rope-scroll absolute inset-y-0 ${ROUTE_LAYOUT[routeIndex]}`}
      style={{
        width: ROPE_WIDTH_PX,
  ...
  const holdNodes = routes.map((route, routeIndex) => (
    <div key={route.key} className={`absolute inset-y-0 ${ROUTE_LAYOUT[routeIndex]}`}>
      {route.holds.map((hold) => {
        const lateralVw = hold.positionKey === "L" ? -LATERAL_OFFSET_VW : hold.positionKey === "R" ? LATERAL_OFFSET_VW : 0;
        ...
        return (
          <div
            ...
            className="animate-fall absolute top-[-10%]"
            style={{
              left: `${lateralVw + hold.jitterVw}vw`,
              width: hold.size,
              height: hold.size,
